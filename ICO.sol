...
contract RE_PreSale is ReentrancyGuard, Context, Ownable {
    ...
    uint public whitelistedUsers;

    mapping (address => bool) Claimed;
    mapping (address => uint256) CoinPaid;
    mapping (address => uint256) TokenBought;
    mapping (address => uint256) valDrop;
    mapping (address => address) public whitelistedtUser;
    mapping (address => bool) public isWhtelisted;
    ...
    */
    ...
    //Start Pre-Sale
    function startICO(uint256 endDate, uint _minPurchase, uint _maxPurchase, uint _availableTokens, uint256 _softCap, uint256 _hardCap, uint256 _poolPercent) external onlyOwner icoNotActive() {
    ...
    function whitelist(address _address)public onlyOwner{
        whitelistedtUser[_address] = _address;
        isWhtelisted[_address] = true;
    }
    // uint public toeknscount = _token.balanceOf(address(this));
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        ...
        require(isWhtelisted[beneficiary] == true,'Not Whitelisted User');
        uint256 weiAmount = msg.value;
        _preValidatePurchase(beneficiary, weiAmount);
        uint256 tokens = _getTokenAmount(weiAmount);
